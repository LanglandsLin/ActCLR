...
class FT_Processor(Processor):
    ...
    def load_optimizer(self):
        if self.arg.optimizer == 'SGD':
            ...
            param_groups = lrd.param_groups_lrd(self.model, self.arg.weight_decay,
                layer_decay=self.arg.layer_decay
            )
    ...
    def adjust_lr(self):
        if self.arg.optimizer == 'SGD' and self.arg.step:
            ...
            for param_group in self.optimizer.param_groups:
                param_group['lr'] = lr
            self.lr = lr
        elif self.arg.optimizer == 'Adam' or self.arg.optimizer == 'AdamW':
            self.lr = lr_sched.adjust_learning_rate(self.optimizer, self.meta_info['epoch'], self.arg)
        else:
    ...
    def confusion_matrix(self):
        predict = np.argmax(self.result, axis=1)
        matrix = np.zeros((60, 60))
        for i, j in zip(self.label, predict):
            matrix[i, j] += 1
        matrix = matrix / np.sum(matrix, axis=1, keepdims=True)
        for i in range(60):
            for j in range(60):
                if matrix[i, j] > 0.1:
                    self.io.print_log('\t{} {}: {:.2f}%'.format(i, j, matrix[i, j]))
    ...
    def train(self, epoch):
        ...
        for data, label in loader:
            ...
            if self.arg.stream == 'joint':
                ...
                data = bone
            elif self.arg.stream == 'angle':
                Bone = [(1, 2), (2, 21), (3, 21), (4, 3), (5, 21), (6, 5), (7, 6), (8, 7), (9, 21),
                        (10, 9), (11, 10), (12, 11), (13, 1), (14, 13), (15, 14), (16, 15), (17, 1),
                        (18, 17), (19, 18), (20, 19), (21, 21), (22, 23), (23, 8), (24, 25), (25, 12)]

                bone = torch.zeros_like(data)
                angle = torch.zeros_like(data)

                for v1, v2 in Bone:
                    bone[:, :, :, v1 - 1, :] = data[:, :, :, v1 - 1, :] - data[:, :, :, v2 - 1, :]

                angle = F.normalize(bone, dim=1)
    ...
    def get_parser(add_help=False):
        ...
        parser.add_argument('--min_lr', type=float, default=0.01, help='initial learning rate')
        parser.add_argument('--step', type=int, default=[], nargs='+', help='the epoch where optimizer reduce the learning rate')
        parser.add_argument('--optimizer', default='SGD', help='type of optimizer')
        parser.add_argument('--nesterov', type=str2bool, default=True, help='use nesterov or not')
        parser.add_argument('--weight_decay', type=float, default=0.0001, help='weight decay for optimizer')
        parser.add_argument('--layer_decay', type=float, default=0.0001, help='weight decay for optimizer')
        ...
        parser.add_argument('--warmup_epochs', type=int, default=1e6,
                            help='the starting epoch of nearest neighbor mining')
