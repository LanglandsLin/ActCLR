...
class AimCLR(nn.Module):
    def __init__(self, base_encoder=None, pretrain=True, feature_dim=128, queue_size=32768,
        ...
        # self.prompt = nn.Parameter(torch.randn(2, 3, 5, 25, 2))
        ...
        if not self.pretrain:
            ...
            if mlp:  # hack: brute-force replacement
                ...
                self.encoder_q.fc = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1),
                                                  nn.ReLU(),
                                                  self.encoder_q.fc)
                self.encoder_k.fc = nn.Sequential(nn.Conv2d(hidden_dim, hidden_dim, kernel_size=1),
                                                  nn.ReLU(),
                                                  self.encoder_k.fc)
